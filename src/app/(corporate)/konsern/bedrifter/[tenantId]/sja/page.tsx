...
export default async function TenantSjaPage({ params }: PageProps) {
  const { tenantId } = await params;
  const analyses = await getGroupTenantSjaAnalyses(tenantId);

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-900">Sikker Jobb-Analyser (SJA)</h2>
        <span className="text-sm text-gray-500">{analyses.length} totalt</span>
      </div>
