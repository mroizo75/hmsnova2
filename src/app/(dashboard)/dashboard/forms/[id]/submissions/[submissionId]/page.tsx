...
function renderFieldValue(
  ...
  if (fileKey) {
    ...
    return (
      <a
        href={`/api/files/${fileKey}`}
        download
        className="inline-flex items-center gap-2 text-primary underline hover:opacity-70"
      >
  ...
  if (!value) {
    return <span className="text-muted-foreground italic">Ikke besvart</span>;
  }
    ...
    if (options && options.length > 0) {
      try {
        ...
        if (selected.length === 0) {
          return <span className="text-muted-foreground italic">Ingen valgt</span>;
        }
        return (
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-bold">{submission.formTemplate.title}</h1>
          {submission.formTemplate.description && (
            <p className="text-muted-foreground mt-1">{submission.formTemplate.description}</p>
          )}
        </div>
      ...
      <div className="space-y-4">
        {submission.formTemplate.fields.map((field, index) => {
            ...
            <Card key={field.id}>
              <CardHeader className="pb-2">
                <CardTitle className="text-base font-semibold flex items-center gap-2">
                  <span className="text-xs font-normal text-muted-foreground w-5 shrink-0">
                    {index + 1}.
                  </span>
                  {field.label}
                  {field.isRequired && (
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <p className="text-sm text-muted-foreground">
                Signert:{" "}
                {format(new Date(submission.signedAt), "d. MMMM yyyy HH:mm", { locale: nb })}
              </p>
              {meta?.signatureData && (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={meta.signatureData}
                  alt="Signatur"
                  className="border rounded-md max-w-xs bg-white"
                />
              )}
            </CardContent>
          </Card>
