...
import { isBcmContinuityAudit } from "@/lib/bcm-audit";

export default async function BcmExerciseDetailPage({
  params,
}: {
  ...
  if (!isBcmContinuityAudit(initialData.audit)) {
    redirect(`/dashboard/audits/${id}`);
  }

  return <AuditDetailContent initialData={initialData} auditId={id} locale={locale} from="bcm" />;
}
