...
type RuhData = NonNullable<Awaited<ReturnType<typeof fetchRuhDetail>>>;

interface RuhDetailContentProps {
  initialData: RuhData;
  ruhId: string;
}

export function RuhDetailContent({ initialData, ruhId }: RuhDetailContentProps) {
  const { data: report } = useQuery({
    queryKey: ["incidents", ruhId],
    queryFn: () => fetchRuhDetail(ruhId),
    initialData,
  });
  ...
  const images = report.attachments.filter((a: any) => a.mime.startsWith("image/"));
  ...
  return (
    <>
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <RuhPDFExport ruhId={report.id} ruhNummer={report.ruhNummer} />
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {report.injuryOccurred && <Badge variant="destructive">Personskade</Badge>}
      </div>
