...
type InspectionMobileData = NonNullable<Awaited<ReturnType<typeof fetchInspectionMobile>>>;

interface InspectionMobileContentProps {
  initialData: InspectionMobileData;
  inspectionId: string;
}

export function InspectionMobileContent({ initialData, inspectionId }: InspectionMobileContentProps) {
  const t = useTranslations("dashboardInspectionMobilePage");

  const { data: inspection } = useQuery({
    queryKey: ["inspections", inspectionId],
    queryFn: () => fetchInspectionMobile(inspectionId),
    initialData,
  });
          ...
          <div className="bg-white rounded-lg border p-6 mb-6">
            ...
            {inspection.formSubmission ? (
              <div className="space-y-4">
                ...
                <Link href={`/dashboard/inspections/${inspectionId}`}>
                  <Button variant="outline" className="w-full">{t("form.view")}</Button>
                </Link>
              </div>
            ) : (
              <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          ...
          <div className="bg-white rounded-lg border p-4 text-center">
            <div className="text-3xl font-bold text-orange-600">
              {inspection.findings.filter((f: any) => f.status === "OPEN").length}
            </div>
