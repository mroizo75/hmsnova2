...
import { AlertTriangle, Download, CheckCircle, Calendar, MapPin, Package } from "lucide-react";
...
type ChemicalDetailData = NonNullable<Awaited<ReturnType<typeof fetchChemicalDetail>>>;

interface ChemicalDetailContentProps {
  initialData: ChemicalDetailData;
}

export function ChemicalDetailContent({ initialData }: ChemicalDetailContentProps) {
  const { data } = useQuery({
    queryKey: ["chemicals", initialData.chemical.id],
    queryFn: () => fetchChemicalDetail(initialData.chemical.id),
    initialData,
  });

  if (!data) return null;

  const { chemical, exposureCount } = data;
    ...
    <>
      ...
      {chemical.containsIsocyanates && (
        <IsocyanateWarning details={chemical.aiExtractedData ? (() => {
          try {
            const d = JSON.parse(chemical.aiExtractedData);
            return d.isocyanateDetails;
          } catch {
            return undefined;
          }
        })() : undefined} />
                    ...
                    <div className="flex flex-wrap gap-3">
                      {pictograms.map((file: string, idx: number) => (
                        <div key={idx} className="relative w-20 h-20 border-2 border-orange-200 rounded-lg p-1">
                          <Image src={`/faremerker/${file}`} alt="Faresymbol" fill className="object-contain" />
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {ppeList.map((file: string, idx: number) => {
                    ...
                    return (
                      <div key={idx} className="relative w-16 h-16 border-2 border-blue-200 rounded-lg p-1 bg-blue-50">
                        <Image src={`/ppe/${normalizedFile}`} alt="PPE-krav" fill className="object-contain" unoptimized />
                      </div>
                    );
                  })}
                </div>
