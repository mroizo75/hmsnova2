...
export function RiskAssessmentItemForm({
  ...
  const [eksisterendeKontroller, setEksisterendeKontroller] = useState("");
  ...
  const handleGenerateAi = async () => {
    ...
    try {
      ...
      setEksisterendeKontroller(result.data.eksisterendeKontroller ?? "");
      ...
      setAiSuggestedTitle(result.data.title);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await addRiskAssessmentItem({
        ...
        eksisterendeKontroller: eksisterendeKontroller.trim() || null,
        suggestedMeasures,
        aiSuggestedTitle: aiSuggestedTitle ?? undefined,
      });
      if (result.success) {
    ...
    <Card>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="item-eksisterende-kontroller">Eksisterende kontroller / barrierer</Label>
            <Textarea
              id="item-eksisterende-kontroller"
              value={eksisterendeKontroller}
              onChange={(e) => setEksisterendeKontroller(e.target.value)}
              placeholder="Hvilke rutiner, verneutstyr eller opplæring er allerede på plass for denne risikoen?"
              disabled={loading}
              rows={3}
            />
          </div>
