...
import { Edit, TrendingUp, ListTodo, User, Calendar } from "lucide-react";
...
type GoalDetailData = NonNullable<Awaited<ReturnType<typeof fetchGoalDetail>>>;

interface GoalDetailContentProps {
  initialData: GoalDetailData;
  currentUserId: string;
}

export function GoalDetailContent({ initialData, currentUserId }: GoalDetailContentProps) {
  const { data } = useQuery({
    queryKey: ["goals", initialData.goal.id],
    queryFn: () => fetchGoalDetail(initialData.goal.id),
    initialData,
  });

  if (!data) return null;

  const { goal, owner } = data;
