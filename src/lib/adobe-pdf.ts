...
import { REPORT_TEMPLATES_DIR } from "@/lib/report-merge-data";
...
export function getReportTemplatePath(filename: string): string {
  return path.join(process.cwd(), REPORT_TEMPLATES_DIR, filename);
}

/**
 * Fyll en Word-mal med JSON via Adobe Document Generation og returner PDF.
 * AML/IK-HMS-rapporter bruker dette for pixel-korrekt layout.
 */
export async function generatePdfFromTemplate(
  ...
  const templatePath = getReportTemplatePath(templateFilename);
  if (!fs.existsSync(templatePath)) {
    throw new Error(`Rapportmal mangler: ${templateFilename}`);
  }
